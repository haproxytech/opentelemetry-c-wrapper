 ...
 *
 *   The caller must drain every concurrent operation on this meter instance
 ...
 *   create_mutex, and such a snapshot racing with the replacement is a data
 *   race.  The instruments created before a restart stay bound to the replaced
 *   pipeline, while every recorded view is registered with the new provider
 *   again, so the views keep applying to the instruments created after the
 *   restart.
 *
 * RETURN VALUE
 *   Returns OTELC_RET_OK on success, or OTELC_RET_ERROR in case of an error.
