 ...
 *   Bucket boundaries are accepted only for a histogram instrument type, as
 *   a non-empty array with a matching count.  If a view with the same name
 *   already exists, its ID is returned and no new view is created.  The
 *   specification of every view is recorded, so a restart of the meter
 *   registers the views with its new provider again.
 *
 * RETURN VALUE
...
static int64_t otel_meter_create_instrument(struct otelc_meter *meter, const char *name, const char *desc, const char *unit, otelc_metric_instrument_t type, struct otelc_metric_observable_cb *data)
{
	...
	/* Only a new instrument needs the descriptor; an existing one ignores it. */
...
static int otel_meter_instrument_value_type(struct otelc_meter *meter, const struct otelc_value *value, otelc_metric_instrument_t type)
{
	...
	if ((expected_type == OTELC_VALUE_NULL) || (value->u_type == expected_type))
		...
		OTEL_METER_RETURN_INT("Value type mismatch: instrument expects %d, got %d", expected_type, value->u_type);
	else if (value->u.value_int64 < 0)
		OTEL_METER_RETURN_INT("Negative int64 value cannot be used for a uint64 instrument");
...
static int otel_meter_start(struct otelc_meter *meter)
{
	...
	OTEL_YAML_PATH(path_e, meter, "/scope_name");
	...
	if ((retval = otel_meter_provider_create(meter, readers, provider)) != OTELC_RET_ERROR) {
		...
		/***
		 * A new provider knows no view, so each recorded view is registered
		 * again before the provider replaces the installed one; a failed
		 * registration leaves the previous pipeline in place.
		 */
...
const static struct otelc_meter_ops otel_meter_ops = {
	...
	.start                      = otel_meter_start,                      /* lock create_mutex */
...
struct otelc_meter *otelc_meter_create(const struct otelc_ctx *ctx, char **err)
{
	...
	if (yaml_resolve_prefix(ctx->fyd, err, OTEL_YAML_METER_PREFIX, ctx->name, OTEL_YAML_NAME_DEFAULT, &(retptr->yaml_prefix)) == OTELC_RET_ERROR) {
