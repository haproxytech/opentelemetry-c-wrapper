...
struct otelc_tracer *otelc_tracer_create(const struct otelc_ctx *ctx, char **err)
{
	...
	/***
	 * Register this tracer with the tracer count so the shared span and
	 * span-context maps stay alive until the last tracer is destroyed.
	 * Incremented before any cleanup path that calls otel_tracer_destroy()
	 * so the matching decrement balances out.
	 */
