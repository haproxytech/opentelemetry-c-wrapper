...
static void otel_tracer_destroy(struct otelc_tracer **tracer)
{
	 ...
	 *
	 * Caveat: the otel_span and otel_span_context handle maps are shared
	 * by every tracer in the process (or, in the thread-local build, by
	 * every tracer on the current thread).  The cleanup below ends and
	 * discards all entries, including spans owned by other tracers.
	 * Callers that run multiple tracers concurrently must end any
	 * cross-tracer spans before destroying any one tracer if those
	 * spans are to be preserved.
	 */
