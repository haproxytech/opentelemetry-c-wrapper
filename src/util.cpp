 ...
 *   handle maps; the instrument and view counters are read from the meter's
...
void otelc_lib_shutdown(void)
{
	...
	/***
	 * A map reads the count only when it is constructed, so no live map is
	 * touched: the dynamic build tore its maps down with the last tracer,
	 * and the static builds keep their maps with the original count.
	 */
