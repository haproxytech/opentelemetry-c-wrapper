 ...
 *   cannot be truncated, and that failure is ignored.  Both take the mutex the
 *   exporter methods hold, because the worker thread of the pipeline is already
 *   running when the start operation calls them.
 *
 * RETURN VALUE
 *   open() returns true when the file is open, false otherwise; the constructor
 *   and truncate() do not return a value.
 */
otel_file_exporter::otel_file_exporter(const char *filename)
	: mutex_(), file_(), filename_()
{
 ...
 *   in the stream buffer.  Every method that reaches the stream holds the mutex
 *   of the base, which orders it against the open and the truncate of the start
 *   operation; MakeRecordable() leaves the stream alone and takes nothing.  The
 *   file stream lives in the base constructed before the wrapped exporter, so
 *   it outlives every write the exporter performs.
 */
