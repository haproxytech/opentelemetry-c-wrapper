 ...
 *   static int otel_exporter_set_otlp_file_options(const struct otelc_ctx *ctx, const char *desc, const char *path, T &options, R &rt_options, char **err, const char *name)
 *
 * ARGUMENTS
 ...
 */
template <typename T, typename R>
static int otel_exporter_set_otlp_file_options(const struct otelc_ctx *ctx, const char *desc, const char *path, T &options, R &rt_options, char **err, const char *name = nullptr)
{
	/***
 ...
 * SYNOPSIS
 *   template <typename T>
 *   static int otel_exporter_set_otlp_grpc_options(const struct otelc_ctx *ctx, const char *desc, const char *path, char *endpoint, T &options, char **err, const char *name)
 *
 * ARGUMENTS
 *   ctx      - library context providing the YAML configuration
 *   desc     - description of the exporter
 ...
 */
template <typename T>
static int otel_exporter_set_otlp_grpc_options(const struct otelc_ctx *ctx, const char *desc, const char *path, char *endpoint, T &options, char **err, const char *name = nullptr)
{
	char    thread_name[OTEL_YAML_BUFSIZ] = "";
 ...
 */
template <typename T, typename R>
static int otel_exporter_set_otlp_http_options(const struct otelc_ctx *ctx, const char *desc, const char *path, char *endpoint, T &options, R &rt_options, int64_t *thread_wait_time, char **err, const char *name = nullptr)
{
	/***
	 * The default values are defined in the include file
...
int otel_tracer_exporter_create(struct otelc_tracer *tracer, std::unique_ptr<otel_sdk_trace::SpanExporter> &exporter, const char *name)
{
	...
	OTEL_YAML_PATH(path, tracer->yaml_prefix, OTEL_YAML_EXPORTERS);
	...
	OTEL_EXPORTER_CASE_OSTREAM(TRACER, otel_exporter_trace::OStreamSpanExporter, tracer->ctx, path, otel_tracer_logfile, &(tracer->err))
	OTEL_EXPORTER_CASE_OTLP_FILE(TRACER, OtlpFileExporter, tracer->ctx, path, &(tracer->err))
	OTEL_EXPORTER_CASE_OTLP_GRPC(TRACER, OtlpGrpcExporter, tracer->ctx, path, &(tracer->err))
	OTEL_EXPORTER_CASE_OTLP_HTTP(TRACER, OtlpHttpExporter, tracer->ctx, path, &(tracer->err))
...
int otel_meter_exporter_create(struct otelc_meter *meter, std::unique_ptr<otel_sdk_metrics::PushMetricExporter> &exporter, const char *name)
{
	...
	OTEL_YAML_PATH(path, meter->yaml_prefix, OTEL_YAML_EXPORTERS);
...
//		const auto data = std::shared_ptr<otel_exporter_memory::InMemoryMetricData>{new(std::nothrow) otel_exporter_memory::SimpleAggregateInMemoryMetricData()};
	...
	OTEL_EXPORTER_CASE_OSTREAM(METER, otel_exporter_metrics::OStreamMetricExporter, meter->ctx, path, otel_meter_logfile, &(meter->err))
	OTEL_EXPORTER_CASE_OTLP_FILE(METER, OtlpFileMetricExporter, meter->ctx, path, &(meter->err))
	OTEL_EXPORTER_CASE_OTLP_GRPC(METER, OtlpGrpcMetricExporter, meter->ctx, path, &(meter->err))
	OTEL_EXPORTER_CASE_OTLP_HTTP(METER, OtlpHttpMetricExporter, meter->ctx, path, &(meter->err))
	else if (strcasecmp(type, OTEL_EXPORTER_ZIPKIN) == 0) {
...
int otel_logger_exporter_create(struct otelc_logger *logger, std::unique_ptr<otel_sdk_logs::LogRecordExporter> &exporter, const char *name)
{
	...
	OTEL_YAML_PATH(path, logger->yaml_prefix, OTEL_YAML_EXPORTERS);
	...
	OTEL_EXPORTER_CASE_OSTREAM(LOGGER, otel_exporter_logs::OStreamLogRecordExporter, logger->ctx, path, otel_logger_logfile, &(logger->err))
	OTEL_EXPORTER_CASE_OTLP_FILE(LOGGER, OtlpFileLogRecordExporter, logger->ctx, path, &(logger->err))
	OTEL_EXPORTER_CASE_OTLP_GRPC(LOGGER, OtlpGrpcLogRecordExporter, logger->ctx, path, &(logger->err))
	OTEL_EXPORTER_CASE_OTLP_HTTP(LOGGER, OtlpHttpLogRecordExporter, logger->ctx, path, &(logger->err))
	else if (strcasecmp(type, OTEL_EXPORTER_ZIPKIN) == 0) {
