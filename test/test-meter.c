 ...
 *   entries, boundaries for an instrument type that is not a histogram, and
 *   boundaries for an aggregation that is neither the default nor the histogram
 *   one, each with a message.
 *
 * RETURN VALUE
 *   This function does not return a value.
 */
static void test_add_view_invalid_args(struct otelc_meter *meter)
{
	...
	/* Boundaries for an aggregation other than default or histogram. */
	OTELC_SFREE_CLEAR(meter->err);
	if ((OTELC_OPS(meter, add_view, "bv_aggr_kind", "", "bv_src", "", hist, sum, bounds, 2) != OTELC_RET_ERROR) || _NULL(meter->err))
		retval = TEST_FAIL;
...
static void test_meter_set_flush_timeout(struct otelc_ctx *ctx)
{
	...
	if (_NULL(meter) || (OTELC_OPS(meter, start) != OTELC_RET_OK)) {
		...
		/* Give the zero-budget destroy a recorded value to drop. */
