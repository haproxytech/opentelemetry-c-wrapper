 ...
 * block whose record a full table has reused since, or of a block that was
 * never tracked because no tracker existed, or it was disabled or its table
 * was full when the block was allocated: no record holds the base of such a
 * block, so it reaches the C library.  Blocks of the C library may be
 * released and reallocated through the wrapper, and that mixing is covered;
 * the other direction is not, because a block of the wrapper is not an
 * address the C library handed out, so free() or realloc() of the C library
 * abort on it in the debug build.
 */
...
/*
 * glibc keeps up to seven released chunks of one size in its tcache and
 * puts the next ones on the fastbin, which overwrites only the first eight
 * header bytes and leaves the magic in place; this count fills the tcache
 * and leaves two blocks behind it.
 */
 ...
 *
 * DESCRIPTION
 *   Disables the tracker that is currently installed, destroys the mutex the
 *   previous test left in the test tracker and initializes the tracker with
 *   the first count records of the static table, so every test starts from
 *   zeroed counters and unused records.
 *
 * RETURN VALUE
...
static int mem_setup(size_t count)
{
	...
	tracker_ready = (otelc_dbg_mem_init(&tracker, table, count) == OTELC_RET_OK);

	return tracker_ready ? TEST_PASS : TEST_FAIL;
}
 ...
 *   the last one again.  The size and count select where the C library puts
 *   the block on the first release, which decides how much of the header
 *   survives: the tcache overwrites the whole header, the fastbin only its
 *   first eight bytes, and a block merged into the top chunk keeps it intact,
 *   so each case takes another rejection path.  The second release must be
 *   rejected: the process survives, the free counter does not move and the
 *   record stays released.
 *
 * RETURN VALUE
 *   This function does not return a value.
 */
 ...
 *
 * SYNOPSIS
 *   static void test_realloc_released(const char *name, size_t size)
 *
 * ARGUMENTS
 *   name - name of the test case
 ...
 *
 * DESCRIPTION
 *   Releases a block of the given size and reallocates it afterwards.  The
 *   size decides which check rejects the call: the C library overwrites the
 *   header of a small block, so the block is found by its base among the
 *   records, while a block merged into the top chunk keeps its header and is
 *   rejected by the record it still points to.  The reallocation must be
 *   rejected with a null result while the counters and the record stay as the
 *   release left them.
 *
 * RETURN VALUE
 *   This function does not return a value.
...
static void test_realloc_released(const char *name, size_t size)
{
	...
	p = OTELC_MALLOC(__func__, __LINE__, size);
	if (_nNULL(p)) {
		...
		if (_nNULL(r) && !r->used) {
			...
			q = OTELC_REALLOC(__func__, __LINE__, p, 2 * size);
...
int main(int argc, char **argv)
{
	...
	test_double_free("double free of a small block behind a full tcache is rejected", 40, MEM_TCACHE_FILL);
	test_double_free("double free of a medium block with an intact header is rejected", 40000, 1);
	test_realloc_released("reallocation of a released small block is rejected", 40);
	test_realloc_released("reallocation of a released medium block is rejected", 40000);
	test_header_lost();
