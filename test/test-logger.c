...
static void test_logger_enabled(struct otelc_logger *logger)
{
	...
	/* An invalid severity is an error for the query and the setter. */
 ...
 *
 * DESCRIPTION
 *   Verifies that shutdown() completes without errors with an unlimited timeout
 *   (NULL) and that a repeated call with a specific timeout value returns,
...
static void test_log_invalid_args(struct otelc_logger *logger)
{
	...
	const struct otelc_value body_bad = { .u_type = (otelc_value_type_t)9 }; /* A wider value would be an invalid enum load. */
	const struct timespec    ts_obs = { .tv_sec = 1, .tv_nsec = 0 };
	int                      rc, retval = TEST_PASS;

	/* Identifier buffers shorter than the SDK size leave no correlation. */
...
static void test_logger_set_flush_timeout(struct otelc_ctx *ctx)
{
	...
	if (_NULL(logger) || (OTELC_OPS(logger, start) != OTELC_RET_OK)) {
		...
		/* An emitted record runs the pipeline before the destroy. */
...
int main(int argc, char **argv)
{
	...
	test_skip("logger tests", "the build lacks the ostream or the OTLP file exporter");
	...
	if (otelc_statistics_check(NULL, 0, 0, 1, 0, 1, 1) != 0)
		retval = TEST_FAIL;
