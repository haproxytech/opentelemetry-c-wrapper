 ...
 *
 * DESCRIPTION
 *   Creates two meters against distinct library contexts, creates an instrument
 *   with the same name and type on each, and records both returned instrument
 *   IDs.  The library's instrument handle map is currently process-wide rather
 *   than per-meter, and so the second create_instrument call returns the first
 *   meter's ID instead of allocating a fresh one bound to the SDK provider
 *   of the second meter.  This test asserts that observed behaviour so a future
 *   change that scopes the handle map per-meter will fail loudly right here and
 *   prompt a corresponding update to the documentation.
 *
 * RETURN VALUE
 *   This function does not return a value.
 */
