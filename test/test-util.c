 ...
 *
 * DESCRIPTION
 *   Scans the output of the ostream metric exporter for the block whose
 *   instrument name equals the given one and returns the number found on the
 *   next "value" line.  When the file holds several exports of the same
 *   instrument, the value of the last one wins, which for a cumulative counter
 *   is the final total.
 *
 * RETURN VALUE
...
int64_t test_metric_value(const char *path, const char *instrument)
{
	...
	for (line = content; _nNULL(line); line = next) {
		...
		if (_nNULL(strstr(line, "instrument name"))) {
			/* Exact match: a name must not hide inside a longer one. */
			sep = strchr(line, ':');
			in_block = _nNULL(sep) && (strcmp(sep + 1 + strspn(sep + 1, " \t"), instrument) == 0);
		}
