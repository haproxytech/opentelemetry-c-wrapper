...
#
# Build a minimal dependency bundle: AWS-LC and curl are built first, and the
# OpenTelemetry C++ SDK follows, with its own cmake configuration downloading
# and compiling the third-party libraries it needs.  curl is compiled against
# AWS-LC on purpose, so that the whole bundle shares one SSL library and the
# curl of the system, tied to the system's SSL library, stays out of it.  The
