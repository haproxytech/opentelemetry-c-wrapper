...
#
# Assembles a self-contained opentelemetry-cpp 1.28.0 source tree along with
...
#
   SH_ARG_DIR="${1:+${1}/}"
SH_OPT_REMOTE="${2:-}"
  SH_REPO_DIR="opentelemetry-cpp-1.28.0"
  ...
  SH_TAG_REPO="
	...
	main   v1.10.0     - open-telemetry/opentelemetry-proto third_party/opentelemetry-proto
...
sh_get()
{
	...
	git ${SH_GIT_OPTS} "${_arg_tag}" "https://github.com/${_arg_repo}.git" "${_arg_dir}"
	cd "${_arg_dir}" || return
...
git ${SH_GIT_OPTS} v1.28.0 https://github.com/open-telemetry/opentelemetry-cpp.git "${SH_ARG_DIR}${SH_REPO_DIR}"
cd "${SH_ARG_DIR}${SH_REPO_DIR}" || exit
git checkout -b main v1.28.0
test -n "${SH_OPT_REMOTE}" || git remote remove origin
