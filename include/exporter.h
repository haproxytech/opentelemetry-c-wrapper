 ...
 *
 * The stream is reached from two sides: the start operation opens and empties
 * it, while an SDK worker thread writes and flushes it, and a periodic metric
 * reader does so on its own timer whether anything was recorded or not.  The
 * mutex below orders the two, since the pipeline the worker belongs to already
 * exists when the start opens the file.
 */
 ...
 * buffer.  Every method that reaches the stream holds the mutex of the base,
 * so the writes of a worker thread are ordered against the open and the
 * truncate the start operation performs.
 *
 * The start operations find the second base with a dynamic_cast, so one loop
 * serves the exporters of every signal.  The SDK exporter interface is fixed
 * and cannot carry the file itself, so the warning about the multiple bases
 * is turned off for the declarations below.
 */
PRAGMA_DIAG_IGNORE("-Wmultiple-inheritance")
 ...
 */
  ...
  #define OTEL_EXPORTER_CASE_OTLP_HTTP(arg_sig, arg_base, arg_ptr, arg_path)                                                            \
		...
		int64_t                                      background_thread_wait_for = 0;                                            \
		                                                                                                                        \
		if (otel_exporter_set_otlp_http_options((arg_ptr)->ctx, OTEL_##arg_sig##_EXPORTER_DESC,                                 \
		                                        (arg_path),                                                                     \
