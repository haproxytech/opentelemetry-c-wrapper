...
/***
 * Downcast helpers that extract the concrete SDK provider pointer out of a
 ...
 * itself rather than the impl, so callers can pass in a local snapshot whose
 * lifetime is bounded by the call instead of the impl member that a concurrent
 * destroy may reset.
 */
