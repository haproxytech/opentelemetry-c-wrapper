...
/***
 * Downcast helpers used by OTEL_PROVIDER_OP to obtain the concrete SDK provider
 * pointer from the API-level provider held in the per-instance implementation
 * state, so that SDK-only operations such as ForceFlush() and Shutdown() can be
 * invoked.
 */
...
/***
 * Generates a provider operation function (ForceFlush or Shutdown) that reaches
 * the per-instance OpenTelemetry SDK provider held in the implementation state
 * of the instance.
 */
