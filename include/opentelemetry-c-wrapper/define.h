 ...
 * nested functions.  The GCC OTELC_DEFER(expr) form still remains available for
 ...
 *
 * The wrapper is required because the C standard makes calling any function
 ...
 *
 * Portability:
 * - The implementation relies on a GCC nested function whose address is then
