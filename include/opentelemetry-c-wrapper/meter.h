...
struct otelc_meter_ops {
	 ...
	 *   with the default or the histogram aggregation, as a non-empty array
	 *   with a matching count.  If a view with the same name already exists,
	 *   its ID is returned and no new view is created.  The specification of
	 *   every view is recorded, so a restart of the meter registers the views
	 *   with its new provider again.
	 *
	 * RETURN VALUE
	 ...
	 *
	 * DESCRIPTION
	 *   Reads the configuration from the /signals/metrics/<name> subtree of
	 *   the YAML document owned by the meter's context and starts the meter.
	 *   The function initializes the meter in such a way that the following
