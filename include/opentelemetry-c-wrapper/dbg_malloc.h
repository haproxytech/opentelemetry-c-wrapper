...
#  define OTELC_STRDUP(f,l,s)      OTELC_DBG_IFDEF(otelc_dbg_strdup(f, l, (s)),       otelc_strdup(s)        )
#  define OTELC_STRNDUP(f,l,s,n)   OTELC_DBG_IFDEF(otelc_dbg_strndup(f, l, (s), (n)), otelc_strndup((s), (n)))
...
#  define OTELC_MEMINFO()          OTELC_DBG_IFDEF(otelc_dbg_mem_info(),                            while (0)              )
#  define OTELC_STRDUP(s)          OTELC_DBG_IFDEF(otelc_dbg_strdup(__func__, __LINE__, (s)),       otelc_strdup(s)        )
#  define OTELC_STRNDUP(s,n)       OTELC_DBG_IFDEF(otelc_dbg_strndup(__func__, __LINE__, (s), (n)), otelc_strndup((s), (n)))
...
struct otelc_dbg_mem {
	...
	size_t                     reused;                       /* Reuse cursor: index at which the scan for a freed record resumes. */
