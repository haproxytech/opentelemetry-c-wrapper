 ...
 * map allocation or teardown it triggers.  OTEL_LOCK_EXPORTER_FILE takes the
 * mutex of the file stream an ostream exporter owns, which the start operation
 * and an SDK worker thread both reach.  Use them at function scope or
